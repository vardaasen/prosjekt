...
This repo writes decision notes in Norwegian and numbers them `NNNN: Tittel`.
Match the existing notes (`docs/decisions/0001-...` to `0006-...`):
...
{Konteksten og hvorfor et valg trengs.}
...
{Følger, også de ubehagelige.}
```

A short note is fine; drop `Alternativer` when there were none worth
remembering. Add `## Status` (for example `Akseptert`, `Erstattet av 0007`)
